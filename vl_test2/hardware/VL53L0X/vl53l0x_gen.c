...
VL53L0X_RangingMeasurementData_t vl53l0x_data1;//传感器1数据结构
VL53L0X_RangingMeasurementData_t vl53l0x_data2;//传感器2数据结构
VL53L0X_RangingMeasurementData_t vl53l0x_data3;//传感器3数据结构
VL53L0X_RangingMeasurementData_t vl53l0x_data4;//传感器4数据结构
VL53L0X_RangingMeasurementData_t vl53l0x_data5;//传感器5数据结构

uint16_t Distance_data[5]={0,0,0,0,0};//5个传感器距离数据
...
VL53L0X_Error vl53l0x_set_mode(VL53L0X_Dev_t *dev,u8 mode)
{
	 ...
	 //vl53l0x_reset(dev);//复位vl53l0x(频繁切换工作模式容易导致采集距离数据不准，需要加上这一代码)
	 status = VL53L0X_StaticInit(dev);

     if(AjustOK!=0)//已校准好,写入校准值
     {                                                                   
	 ...
	 if(status!=VL53L0X_ERROR_NONE) goto error;
	 HAL_Delay(2);
	 status = VL53L0X_SetMeasurementTimingBudgetMicroSeconds(dev,Mode_data[mode].timingBudget);//设定完整测量时间
	 if(status!=VL53L0X_ERROR_NONE) goto error;
	 HAL_Delay(2);
