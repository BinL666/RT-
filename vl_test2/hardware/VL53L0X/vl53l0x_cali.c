...
//    LED1=0;
	...
	else
	{
	    ...
	    LOG_D("SPADS Calibration Error,Restart this step\r\n");
	    goto spads;
	}
	//设备参考校准---------------------------------------------------
	ref:
	HAL_Delay(10);
	LOG_D("The Ref Calibration Start...\r\n");
	...
	if(Status == VL53L0X_ERROR_NONE)
	{
		...
		LOG_D("The Ref Calibration Finish...\r\n\r\n");
		i=0;
	}
	else
	{
		i++;
		if(i==adjust_num) return Status;
		LOG_D("Ref Calibration Error,Restart this step\r\n");
		goto ref;
	}
	//偏移校准------------------------------------------------
	offset:
	HAL_Delay(10);
	LOG_D("Offset Calibration:need a white target,in black space,and the distance keep 100mm!\r\n");
	LOG_D("The Offset Calibration Start...\r\n");
	
	Status = VL53L0X_PerformOffsetCalibration(dev,CalDistanceMilliMeter,&OffsetMicroMeter);//偏移校准
	if(Status == VL53L0X_ERROR_NONE)
	{
	    LOG_D("CalDistanceMilliMeter = %d mm\r\n",CalDistanceMilliMeter);
		Vl53l0x_adjust.CalDistanceMilliMeter = CalDistanceMilliMeter;
		LOG_D("OffsetMicroMeter = %d mm\r\n",OffsetMicroMeter);
		Vl53l0x_adjust.OffsetMicroMeter = OffsetMicroMeter;
		LOG_D("The Offset Calibration Finish...\r\n\r\n");
		i=0;
	}
	else
	{
		i++;
		if(i==adjust_num) return Status;
		LOG_D("Offset Calibration Error,Restart this step\r\n");
		goto offset;
	}
	//串扰校准-----------------------------------------------------
	xtalk:
	HAL_Delay(20);
	LOG_D("Cross Talk Calibration:need a grey target\r\n");
	LOG_D("The Cross Talk Calibration Start...\r\n");
	Status = VL53L0X_PerformXTalkCalibration(dev,XTalkCalDistance,&XTalkCompensationRateMegaCps);//串扰校准
	if(Status == VL53L0X_ERROR_NONE)
	{
	    LOG_D("XTalkCalDistance = %d mm\r\n",XTalkCalDistance);
		Vl53l0x_adjust.XTalkCalDistance = XTalkCalDistance;
		LOG_D("XTalkCompensationRateMegaCps = %d\r\n",XTalkCompensationRateMegaCps);
		Vl53l0x_adjust.XTalkCompensationRateMegaCps = XTalkCompensationRateMegaCps;
		LOG_D("The Cross Talk Calibration Finish...\r\n\r\n");
		i=0;
	}
	else
	{
		i++;
		if(i==adjust_num) return Status;
		LOG_D("Cross Talk Calibration Error,Restart this step\r\n");
		goto xtalk;
	}
//	LED1=1;
	LOG_D("All the Calibration has Finished!\r\n");
	LOG_D("Calibration is successful!!\r\n");
